...
struct _PLCComm
{
  ...
  gboolean ready; /* Ready for next command */
  gchar line[20];
  gssize line_len;
  GThread *thread;
  GCond cond;
...
struct _PLCCommClass
{
  ...
  /* Signals */
  void (*reply_ready)(PLCComm *comm, PLCCmd *cmd);
};
...
G_DEFINE_TYPE (PLCComm, PLC_comm, G_TYPE_OBJECT)

static void
stop_thread(PLCComm *comm);
  
static void
start_thread(PLCComm *comm);
  

static void
dispose(GObject *gobj)
{
  PLCComm *comm = PLC_COMM(gobj);
  stop_thread(comm);
  ...
  G_OBJECT_CLASS(PLC_comm_parent_class)->dispose(gobj);
}
...
finalize(GObject *gobj)
{
  ...
  G_OBJECT_CLASS(PLC_comm_parent_class)->finalize(gobj);
}
...
static void
PLC_comm_class_init (PLCCommClass *klass)
{
  ...
  signals[REPLY_READY] =
    g_signal_new("reply-ready",
                 ...
                 G_STRUCT_OFFSET(PLCCommClass, reply_ready),
                 NULL, NULL,
...
{
  PLCComm *comm = g_object_new (PLC_COMM_TYPE, NULL);
  comm->io = io;
...
serial_thread(gpointer data)
{
  ...
  while(!g_cancellable_is_cancelled(comm->cancel)) {
    ...
    pending_cmd = g_async_queue_try_pop(comm->cmd_queue);
    if (!pending_cmd) {
      pending_cmd = g_new(PLCCmd, 1);
      pending_cmd->cmd = g_strdup_printf("i%d\n", poll_port);
      pending_cmd->reply = NULL;
      if (++poll_port == NPORTS) poll_port = 0;
    }
...
gboolean
PLC_comm_send(PLCComm *comm,  const gchar *cmd_str, GError **err)
{
